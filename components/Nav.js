...
const Nav = () => {
  ...
  const handleScroll = (e) => {
    if (document.documentElement.scrollTop > 105) {
      // console.log("scroll 2", document.documentElement.scrollTop);
      setScroll(true);
    } else {
      setScroll(false);
    }
  ...
  return (
    <div className={`${navStyles.nav_container}`}>
      <div className={`${scroll ? navStyles.nav_inner_container : ""}`}>
        ...
        <nav className={navStyles.nav}>
          <div className={navStyles.menu_icon}>
            <MenuIcon
              ...
              style={
                toggleMobileNav ? { display: "none" } : { display: "block" }
              }
            />
            <CloseIcon
              onClick={() => setToggleMobileNav(false)}
              style={
                toggleMobileNav ? { display: "block" } : { display: "none" }
              }
            />
          </div>
                  ...
                  <ul>
                    ...
                    >
                      <li
                        className={`${
                          router.asPath ===
                          "/categoryPage?category=Locuri%20de%20vizitat"
                            ? navStyles.category_active
                            : ""
                        }`}
                      >
